...
async fn process() -> Result<(), reqwest::Error> {
    ...
    let github_org: &str = &env::var("GITHUB_REPOSITORY").expect("Missing input parameter: repo"); // return org/reponame
    ...
    println!("Org is {:?}, and repo is {:?}", parsed_org, parsed_repo);
